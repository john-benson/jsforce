...
Metadata.prototype.createAsync = function(type, metadata, callback) {
  ...
  var isArray = isArray(metadata);
  metadata = isArray ? map(metadata, convert) : convert(metadata);
...
Metadata.prototype.create = function(type, metadata, callback) {
  ...
  return this._invoke("createMetadata", { metadata: metadata }).then(function(results) {
    return isArray(results) ? map(results, convertToSaveResult) : convertToSaveResult(results);
  }).thenCall(callback);
};
...
Metadata.prototype.read = function(type, fullNames, callback) {
  return this._invoke("readMetadata", { type: type, fullNames: fullNames }).then(function(res) {
    return isArray(res.records) ? map(res.records, convertToMetadataInfo) : convertToMetadataInfo(res.records);
  }).thenCall(callback);
};
...
Metadata.prototype.updateAsync = function(type, updateMetadata, callback) {
  ...
  var isArray = isArray(updateMetadata);
  updateMetadata = isArray ? map(updateMetadata, convert) : convert(updateMetadata);
...
Metadata.prototype.upsert = function(type, metadata, callback) {
  ...
  return this._invoke("upsertMetadata", { metadata: metadata }).then(function(results) {
    return isArray(results) ? map(results, convertToUpsertResult) : convertToUpsertResult(results);
  }).thenCall(callback);
};
...
Metadata.prototype.describe = function(version, callback) {
  ...
  return this._invoke("describeMetadata", { asOfVersion: version }).then(function(res) {
    res.metadataObjects = isArray(res.metadataObjects) ? res.metadataObjects : [ res.metadataObjects ];
    res.metadataObjects = map(res.metadataObjects, function(mo) {
      if (mo.childXmlNames) {
        mo.childXmlNames = isArray(mo.childXmlNames) ? mo.childXmlNames: [ mo.childXmlNames ];
      }
...
Metadata.prototype.deploy = function(zipInput, options, callback) {
  ...
  if (isObject(zipInput) && isFunction(zipInput.pipe)) {
    var bufs = [];
...
Metadata.prototype.checkDeployStatus = function(id, includeDetails, callback) {
  if (isObject(includeDetails) || isBoolean(includeDetails)) {
...
AsyncResultLocator.prototype.then = function(onResolve, onReject) {
  ...
  return this._results.then(function(results) {
    ...
    results = isArray(results) ? map(results, convertType) : convertType(results);
    if (self._isArray && !isArray(results)) {
      results = [ results ];
    }
...
AsyncResultLocator.prototype.thenCall = function(callback) {
  return isFunction(callback) ? this.then(function(res) {
...
AsyncResultLocator.prototype.check = function(callback) {
  ...
  return this.then(function(results) {
    var ids = isArray(results) ? map(results, function(res){ return res.id; }) : results.id;
    self._ids = ids;
...
AsyncResultLocator.prototype.poll = function(interval, timeout) {
  ...
  var poll = function() {
    ...
    self.check().then(function(results) {
      var done = true;
      var resultArr = isArray(results) ? results : [ results ];
